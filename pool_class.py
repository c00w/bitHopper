...
class Pool():
    ...
    def __lt__(self, other):
        if self.role in ['backup', 'backup_latehop']:
            ...
            rr_self += getattr(self, 'penalty', 0.0)
            rr_other = float(other.rejects/other.user_shares)
            rr_other += getattr(other, 'penalty', 0.0)
    ...
    def btc_shares(self,):
        ...
        if self.role in ['mine_force', 'mine_lp_force']:
            shares = 0
        # apply penalty
        shares = shares * float(getattr(self,'penalty', 1))
        return shares, self
    ...
    def is_valid(self):
        ...
        if self.role not in ['backup', 'backup_latehop'] and self.api_lag:
            return False
        if self.coin not in self.bitHopper.exchange.profitability:
            return False
        if self.bitHopper.exchange.profitability[self.coin] < 1.0:
            return False
        return True
